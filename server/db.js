...
// DEMO ONLY. Which browser started this session.
//
// This is NOT authentication and must not be mistaken for it: the value
// is generated by the client and sent in a header, so anyone can send
// anyone else's. It exists because the hackathon demo is one shared,
// unauthenticated deployment with no login, where without it two
// visitors collide badly — they see each other's cooks listed, and
// starting one run abandons the other's (see abandonOtherActiveStmt).
//
// Anything resembling real multi-user support -- private data, accounts,
// a session that survives a device change -- needs a real identity on
// the server, not this. Delete this column when that arrives.
if (!sessionColumns.includes("client_id")) {
  db.exec("ALTER TABLE sessions ADD COLUMN client_id TEXT");
}
